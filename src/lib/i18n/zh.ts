...
export const zh: Messages = {
  ...
  legal: {
    ...
    privacySections: [
      ...
      { title: "我们处理哪些信息", body: "若您创建账户，我们保存电子邮件与密码哈希（或登录提供方标识）以及角色。联系与配置申请会把姓名、邮箱、留言以及可选的机构、电话或资产类型存为线索——部署数据库可用时写入数据库，否则写入本地文件。仅当本部署配置了邮箱或 webhook 时才会发送副本。浏览器在本地保存语言、主题和 DEMO/LIVE。事件日志、Black Box 与 Pilot 对话先写在设备上；仅在部署数据库已配置时才会同步。我们不用 AGRON 1 或 Pilot 内容训练模型。" },
