...
import { HudGlyph, PilotHex } from "@/components/bridge/hud-icons";
...
export function PilotTalkWindow({
  ...
  return createPortal(
    <div
      ...
      dir="ltr"
      className="fixed inset-0 z-[90] flex items-center justify-center bg-bridge-bg/55 p-3 sm:p-6"
    >
      <HudBezel
        testId="pilot-talk-hud"
        className="pilot-talk-hud flex min-h-[28rem] w-full max-w-6xl flex-col"
        status={status}
        onClose={onClose}
        closeLabel={title}
        ...
        <div className="relative grid min-h-[22rem] gap-4 pb-8 sm:grid-cols-[minmax(0,22rem)_minmax(0,1fr)]">
          <section className="flex min-h-[22rem] flex-col overflow-hidden border border-bridge-line bg-bridge-panel/95 shadow-[0_16px_40px_rgb(0_0_0/0.18)]">
            <header className="flex items-center justify-between border-b border-bridge-line bg-bridge-bg px-3 py-2">
              ...
              <span className="flex items-center gap-1 text-bridge-dim">
              ...
              {recent.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.role === "user" ? (
                      <span className="mb-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-bridge-line bg-bridge-bg text-bridge-dim">
