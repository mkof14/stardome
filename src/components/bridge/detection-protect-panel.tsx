        ...
        return (
          <g key={track.id}>
            <path
              ...
              strokeWidth="1.4"
            />
            <circle cx={sx} cy={sy} r="3.4" fill="#2EE59A" />
          </g>
        );
      })}
    </svg>
        ...
        {LAYER_IDS.map((id) => {
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <HudGlyph
                  ...
                  className={cn(
                    ...
                    hot ? "text-attn" : "text-bridge-text",
                  )}
                />
              ...
              <p className="mt-2 font-body text-sm leading-relaxed text-bridge-dim">{layer.action}</p>
              {bound.length ? <LayerMini tracks={bound} /> : null}
              <p className="mt-2 font-mono text-[11px] tracking-wide text-[#7DCF9A]">
                {first
                  ? `${copy.onPicture} · ${first.trackNo ?? first.id} · ${first.rangeText ?? first.dist}`
                  : copy.boundNone}
              </p>
              <p className={cn("mt-2 inline-flex items-center gap-1.5 font-mono text-[11px] tracking-wider", statusClass(status))}>
